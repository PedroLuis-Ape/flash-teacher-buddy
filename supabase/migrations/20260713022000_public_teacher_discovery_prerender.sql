-- Public discovery feed used by build-time SEO pre-rendering.
-- Only fields already approved for anonymous teacher profiles are exposed.

CREATE OR REPLACE FUNCTION public.list_public_teacher_discovery_entries(
...
AS $$
  ...
  ORDER BY folder_count DESC, list_count DESC, display_name ASC
  LIMIT LEAST(GREATEST(COALESCE(_limit, 500), 1), 5000);
$$;

-- The previous implementation resolved profiles through search_public_teachers('', 24),
-- which made profiles outside the first 24 directory rows impossible to open by slug.
...
REVOKE ALL ON FUNCTION public.list_public_teacher_discovery_entries(integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_public_teacher_profile(text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.list_public_teacher_discovery_entries(integer) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_teacher_profile(text) TO anon, authenticated;

COMMENT ON FUNCTION public.list_public_teacher_discovery_entries(integer) IS
  'Returns only explicitly public teacher profile fields for sitemap generation and static discovery pages.';
