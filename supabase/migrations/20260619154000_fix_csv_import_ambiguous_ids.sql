-- App Piteco — hotfix do importador CSV/legado.
-- Corrige colisões entre variáveis PL/pgSQL e colunas folder_id/list_id.
...
BEGIN
  ...
  END IF;
  IF _payload->>'schema' IS DISTINCT FROM 'appteco-global-import'
     OR COALESCE((_payload->>'version')::integer, 0) <> 1 THEN
  ...
  IF jsonb_array_length(_payload #> '{package,folders}') > 100 THEN
    RAISE EXCEPTION 'Limite de 100 pastas excedido.';
  END IF;
  IF jsonb_typeof(_destination_plan->'folders') IS DISTINCT FROM 'object' THEN
    RAISE EXCEPTION 'Plano de destinos inválido.';
  END IF;

  package_name := NULLIF(BTRIM(_payload #>> '{package,name}'), '');
  IF package_name IS NULL OR char_length(package_name) > 160 THEN
    RAISE EXCEPTION 'Nome do pacote inválido.';
  END IF;
  IF _institution_id IS NOT NULL AND NOT EXISTS (
    SELECT 1
    FROM public.institutions AS institution
    WHERE institution.id = _institution_id
      AND institution.owner_id = uid
  ) THEN
  ...
  SELECT batch.*
  INTO existing
  FROM public.global_import_batches AS batch
  WHERE batch.user_id = uid
    AND batch.request_id = _request_id;

  IF FOUND THEN
    IF existing.payload_hash <> payload_hash THEN
      RAISE EXCEPTION 'request_id já usado com outro pacote.';
    END IF;
    IF existing.status = 'undone' THEN
      RAISE EXCEPTION 'Esta importação já foi desfeita. Inicie outra tentativa.';
    END IF;
    RETURN existing.summary || jsonb_build_object(
  ...
  LOOP
    ...
    IF jsonb_typeof(fp) IS DISTINCT FROM 'object' THEN
      RAISE EXCEPTION '%: destino ausente.', folder_path;
    END IF;

    IF fp #>> '{folder,mode}' = 'existing' THEN
      v_folder_id := (fp #>> '{folder,folderId}')::uuid;
      IF NOT EXISTS (
        SELECT 1
        FROM public.folders AS folder
        WHERE folder.id = v_folder_id
          AND folder.owner_id = uid
      ...
      INSERT INTO public.global_import_items(
        ...
        v_batch_id, uid, 'folder', v_folder_id, 'reused', folder_path
      );
    ELSIF fp #>> '{folder,mode}' = 'create' THEN
      ...
      IF folder_name IS NULL OR char_length(folder_name) > 160 THEN
        RAISE EXCEPTION '%: nome inválido.', folder_path;
      END IF;
      INSERT INTO public.folders(
        ...
        uid,
        folder_name,
        NULLIF(BTRIM(fr.value->>'description'), ''),
        'private',
        _institution_id,
        NULLIF(BTRIM(_payload #>> '{package,source_language}'), ''),
        NULLIF(BTRIM(_payload #>> '{package,target_language}'), '')
      )
      ...
      INSERT INTO public.global_import_items(
        ...
        v_batch_id, uid, 'folder', v_folder_id, 'created', folder_path
      );
    ELSE
      RAISE EXCEPTION '%: modo de pasta inválido.', folder_path;
    END IF;

    IF jsonb_typeof(fr.value->'lists') IS DISTINCT FROM 'array'
       OR jsonb_array_length(fr.value->'lists') = 0 THEN
      RAISE EXCEPTION '%: listas ausentes.', folder_path;
    END IF;

    SELECT COALESCE(MAX(list_row.order_index), -1) + 1
    INTO next_order
    FROM public.lists AS list_row
    WHERE list_row.folder_id = v_folder_id
    ...
    LOOP
      ...
      IF total_lists > 500 THEN
        RAISE EXCEPTION 'Limite de 500 listas excedido.';
      END IF;
      ...
      IF jsonb_typeof(lp) IS DISTINCT FROM 'object' THEN
        RAISE EXCEPTION '%: destino ausente.', list_path;
      END IF;
      IF jsonb_typeof(lr.value->'cards') IS DISTINCT FROM 'array'
         OR jsonb_array_length(lr.value->'cards') = 0 THEN
        RAISE EXCEPTION '%: cards ausentes.', list_path;
      END IF;
      IF lr.value ? 'expected_cards'
         AND (lr.value->>'expected_cards')::integer <> jsonb_array_length(lr.value->'cards') THEN
        RAISE EXCEPTION '%: contagem incorreta.', list_path;
      END IF;
      ...
      IF lp->>'mode' = 'skip' THEN
        ...
        INSERT INTO public.global_import_items(
          ...
          v_batch_id, uid, 'list', NULL, 'skipped', list_path
        );
        CONTINUE;
      ELSIF lp->>'mode' = 'existing' THEN
        v_list_id := (lp->>'listId')::uuid;
        IF NOT EXISTS (
          SELECT 1
          FROM public.lists AS list_row
          WHERE list_row.id = v_list_id
            AND list_row.folder_id = v_folder_id
            AND list_row.owner_id = uid
            AND list_row.deleted_at IS NULL
        ) THEN
        ...
        IF COALESCE(lp->>'strategy', 'append') = 'replace' THEN
          ...
          WHERE flashcard.list_id = v_list_id
            AND flashcard.user_id = uid;

          DELETE FROM public.flashcards AS flashcard
          WHERE flashcard.list_id = v_list_id
            AND flashcard.user_id = uid;
        ...
        INSERT INTO public.global_import_items(
          ...
          v_list_id,
          CASE
            WHEN COALESCE(lp->>'strategy', 'append') = 'replace' THEN 'replaced'
            ELSE 'reused'
          END,
          list_path
        );
      ELSIF lp->>'mode' = 'create' THEN
        ...
        IF list_name IS NULL OR char_length(list_name) > 160 THEN
          RAISE EXCEPTION '%: nome inválido.', list_path;
        END IF;
        ...
        INSERT INTO public.lists(
          ...
          uid,
          list_name,
          NULLIF(BTRIM(lr.value->>'description'), ''),
          next_order,
          'private',
          _institution_id,
          NULLIF(BTRIM(_payload #>> '{package,source_language}'), ''),
        ...
        INSERT INTO public.global_import_items(
          ...
          v_batch_id, uid, 'list', v_list_id, 'created', list_path
        );
      ELSE
        RAISE EXCEPTION '%: modo de lista inválido.', list_path;
      END IF;
      ...
      LOOP
        ...
        IF total_cards > 10000 THEN
          RAISE EXCEPTION 'Limite de 10.000 cards excedido.';
        END IF;
        ...
        IF front_text IS NULL OR back_text IS NULL
           OR char_length(front_text) > 8000
           OR char_length(back_text) > 8000 THEN
          RAISE EXCEPTION '%: conteúdo inválido.', card_path;
        END IF;
        ...
        SELECT EXISTS(
          ...
          WHERE flashcard.list_id = v_list_id
            ...
            AND LOWER(BTRIM(flashcard.term)) = LOWER(front_text)
            AND LOWER(BTRIM(flashcard.translation)) = LOWER(back_text)
        ) INTO duplicate_found;

        IF duplicate_found AND _card_conflict = 'error' THEN
          RAISE EXCEPTION '%: card duplicado.', card_path;
        END IF;
        ...
        IF duplicate_found AND _card_conflict = 'skip' THEN
          ...
          INSERT INTO public.global_import_items(
            ...
            v_batch_id, uid, 'card', NULL, 'skipped', card_path
          );
        ELSE
          INSERT INTO public.flashcards(
            ...
            front_text,
            back_text,
            NULLIF(BTRIM(cr.value->>'hint'), ''),
            NULLIF(BTRIM(cr.value->>'context_tag'), ''),
            NULLIF(BTRIM(cr.value->>'example'), ''),
            NULLIF(BTRIM(cr.value->>'example_translation'), '')
          )
          ...
          INSERT INTO public.global_import_items(
            ...
            v_batch_id, uid, 'card', v_card_id, 'created', card_path
          );
        END IF;
      END LOOP;
    END LOOP;

    IF fr.value ? 'expected_cards'
       AND (fr.value->>'expected_cards')::integer <> folder_cards THEN
      RAISE EXCEPTION '%: contagem incorreta.', folder_path;
    END IF;
  END LOOP;
  ...
  UPDATE public.global_import_batches AS batch
