...
DECLARE
  ...
  v_portal_allowed boolean := false;
BEGIN
  SELECT
    l.owner_id,
    l.visibility::text,
  ...
  -- Keep glossary access in lockstep with the same public portal rule that
  -- already allows anonymous visitors to load this list's flashcards.
  IF NOT v_allowed
     AND to_regprocedure('public.get_portal_flashcards(uuid)') IS NOT NULL THEN
    EXECUTE
      'SELECT EXISTS (SELECT 1 FROM public.get_portal_flashcards($1) LIMIT 1)'
    INTO v_portal_allowed
    USING _list_id;
    v_allowed := COALESCE(v_portal_allowed, false);
  END IF;
