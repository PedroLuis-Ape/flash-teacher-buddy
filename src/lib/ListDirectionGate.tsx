...
export function ListDirectionGate({ children }: { children: ReactNode }) {
  ...
  const gameRoute = location.pathname.includes("/games") || location.pathname.includes("/study");
  ...
  const query = useQuery({
    ...
    enabled: gameRoute && !!listId && !explicit,
    staleTime: 300_000,
  });
  if (!gameRoute || !listId || explicit) return <>{children}</>;
  if (query.isLoading) return <div className="min-h-[50vh] grid place-items-center">Preparando atividade...</div>;
