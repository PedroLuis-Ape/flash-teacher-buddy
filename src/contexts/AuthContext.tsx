...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const clearCacheOnIdentityChange = useCallback((nextSession: Session | null) => {
    ...
    if (previousUserId && previousUserId !== nextUserId) {
      // Never let a query from account A remain visible while account B is
      // hydrating. RLS protects the request, but stale React Query data could
      // still flash in the UI during the identity transition.
      queryClient.clear();
    }
  }, [queryClient]);
