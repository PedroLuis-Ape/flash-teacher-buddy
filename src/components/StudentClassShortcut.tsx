...
export function StudentClassShortcut() {
  const navigate = useNavigate();
  const { data, isLoading } = useTurmasAsAluno();

  const turmas = data?.turmas || [];

  // Don't render anything if loading or no classes
  if (isLoading || turmas.length === 0) return null;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <ApeSectionTitle>Suas Turmas</ApeSectionTitle>
        </div>
        {hasMore && (
          <Button
            variant="ghost"
