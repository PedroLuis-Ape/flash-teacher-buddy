      ...
      <div className="container mx-auto px-4 py-6">
        <Card className="rounded-2xl"><CardContent className="py-10 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Visite a aba Pacotes para começar sua coleção.
          </p>
        </CardContent></Card>
      </div>
    );
    ...
    <div className="container mx-auto space-y-5 px-4 py-5">
            ...
            <Card key={label} className="rounded-2xl">
              <CardContent className="flex min-h-24 items-center gap-3 p-3">
                <div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-full bg-muted">
                  {image ? <img src={image} alt={item?.name || label} className="h-full w-full object-cover" /> :
      ...
      <section>
        ...
        <p className="mb-3 text-sm text-muted-foreground">
          Toque em um pacote para visualizar e equipar.
        </p>
                ...
                <div className="flex min-h-36 sm:block">
                  ...
                  <CardContent className="flex min-w-0 flex-1 flex-col p-3">
                    ...
                    <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">
                      {item.description || "Pacote visual do App Piteco."}
                    </p>
            ...
            <>
              {selected.avatar && (
                <Button
                  ...
                  disabled={busy === `${selected.skin_id}:photo`}
                >
              ...
              <Button
                variant="outline"
                className="h-12 w-full gap-2 sm:w-auto"
                onClick={() => equip(selected, "avatar")}
                disabled={selectedAvatar || busy === `${selected.skin_id}:avatar`}
              >
                {busy === `${selected.skin_id}:avatar` ? <Loader2 className="h-4 w-4 animate-spin" /> :
                  selectedAvatar ? <Check className="h-4 w-4" /> : <UserRound className="h-4 w-4" />}
                {selectedAvatar ? "Avatar equipado" : "Equipar avatar"}
              </Button>
              <Button
                className="h-12 w-full gap-2 sm:w-auto"
                onClick={() => equip(selected, "mascot")}
                disabled={selectedMascot || busy === `${selected.skin_id}:mascot`}
              >
                {busy === `${selected.skin_id}:mascot` ? <Loader2 className="h-4 w-4 animate-spin" /> :
                  selectedMascot ? <Check className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />}
