...
export function AiPromptPresetSelector({ context, smartInterview = false }: Props) {
  ...
  const guidedInterview = smartInterview || isSuperImportTestRolloutEnabled();
  const prompt = useMemo(() => (
    guidedInterview && context
      ? buildOwnerFinalImportPrompt(preset, context)
      : buildFinalGlobalImportPrompt(preset, context)
  ), [preset, context, guidedInterview]);
  ...
  return <Card className="space-y-5 p-5">
    ...
    <div className="grid gap-2 sm:grid-cols-2"><Button type="button" onClick={copyPrompt}><Clipboard className="mr-2 h-4 w-4" />Copiar este prompt</Button><Button type="button" variant="outline" onClick={() => setVisible((value) => !value)}><Eye className="mr-2 h-4 w-4" />{visible ? "Ocultar prompt" : "Visualizar prompt"}</Button></div>
