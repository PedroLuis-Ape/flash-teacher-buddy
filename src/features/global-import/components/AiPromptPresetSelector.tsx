...
const ICONS = {
  batch: Layers3,
  detailed: BookOpenCheck,
  complete: LibraryBig,
} satisfies Record<GlobalImportAiPreset, typeof Layers3>;

export function AiPromptPresetSelector({ context }: Props) {
  const [preset, setPreset] = useState<GlobalImportAiPreset>("batch");
  const [visible, setVisible] = useState(false);
  const prompt = useMemo(() => buildGlobalImportPresetPrompt(preset, context), [preset, context]);
    ...
    <Card className="space-y-5 p-5">
      <div>
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="text-lg font-semibold">Criar conteúdo com IA</h2>
          <Badge variant="secondary">contrato 2.0</Badge>
        </div>
        <p className="mt-1 text-sm text-muted-foreground">
          Escolha o nível de conteúdo. O prompt copiado exige um JSON que o App Piteco consegue validar e importar.
        </p>
      </div>
          ...
          return (
            <button
              ...
              className={`relative rounded-xl border p-4 text-left transition-colors ${
                active ? "border-primary bg-primary/5 ring-1 ring-primary" : "hover:border-primary/40 hover:bg-muted/40"
              }`}
              aria-pressed={active}
            >
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {item.includes.map((label) => <Badge key={label} variant="secondary" className="font-normal">{label}</Badge>)}
              </div>
            </button>
          );
        })}
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <Button onClick={copyPrompt}><Clipboard className="mr-2 h-4 w-4" />Copiar este prompt</Button>
