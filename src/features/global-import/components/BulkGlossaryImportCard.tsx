...
export function BulkGlossaryImportCard({ catalog, turmaId }: Props) {
  ...
  const run = async () => {
    ...
    setReport(null);
    try {
      const preview = await importGlossaryToFolders({
        folderIds: [...selected],
      ...
      if (preview.requires_confirmation) {
        const confirmed = window.confirm([
          "Já existem entradas relacionadas.",
          `${preview.skipped_exact} idêntica(s) serão ignoradas.`,
          `${preview.updated} terão nota ou estado atualizado.`,
          `${preview.added_as_layer} tradução(ões) alternativa(s) serão adicionadas como camada.`,
          "Deseja continuar?",
        ].join("\n"));
        if (!confirmed) {
          setReport(preview);
      ...
      const applied = await importGlossaryToFolders({
        folderIds: [...selected],
        entries: parsed.entries,
        turmaId,
    ...
    <Card className="overflow-hidden">
      <button type="button" onClick={() => setExpanded((value) => !value)} className="flex w-full items-center gap-3 p-5 text-left">
        ...
        <span className="flex-1">
          <strong className="block">Glossário em massa</strong>
          <span className="text-sm text-muted-foreground">Aplique um único glossário a todas as listas das pastas escolhidas.</span>
        </span>
      ...
      {expanded && <div className="space-y-5 border-t p-5">
        <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 text-sm text-muted-foreground">
          Entradas idênticas são ignoradas. Mudanças de nota ou ativação são atualizadas. Traduções diferentes para o mesmo termo permanecem como camadas.
        </div>

        <div className="grid gap-4 md:grid-cols-[1fr_12rem]">
          <div>
            <Label htmlFor="bulk-glossary">Glossário</Label>
            <Textarea id="bulk-glossary" value={raw} onChange={(event) => setRaw(event.target.value)} rows={9} className="mt-2 font-mono text-sm" placeholder={"=== GLOSSÁRIO GLOBAL ===\nI / eu\nam / sou, estou\n=== CARDS ==="} />
          </div>
          <div className="space-y-3">
            <div>
              <Label>Lado padrão</Label>
              <Select value={defaultSide} onValueChange={(value) => setDefaultSide(value as "A" | "B")}>
                <SelectTrigger className="mt-2"><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="A">Lado A</SelectItem><SelectItem value="B">Lado B</SelectItem></SelectContent>
              </Select>
            </div>
            <div className="rounded-lg bg-muted p-3 text-sm">
              <div><strong>{parsed.entries.length}</strong> termo(s)</div>
              <div><strong>{targetedLists}</strong> lista(s)</div>
              <div><strong>{parsed.entries.length * targetedLists}</strong> aplicação(ões)</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <div><Label>Pastas de destino</Label><p className="text-xs text-muted-foreground">Todas as listas existentes serão incluídas.</p></div>
            <Button variant="outline" size="sm" onClick={toggleAll}><CheckSquare className="mr-1.5 h-4 w-4" />{allSelected ? "Desmarcar todas" : "Marcar todas"}</Button>
          </div>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {(catalog?.folders ?? []).map((folder) => <label key={folder.id} className="flex cursor-pointer items-start gap-3 rounded-lg border p-3 hover:bg-muted/50">
              <Checkbox checked={selected.has(folder.id)} onCheckedChange={() => toggleFolder(folder.id)} className="mt-0.5" />
              <span className="min-w-0"><strong className="block truncate text-sm">{folder.title}</strong><span className="text-xs text-muted-foreground">{listCountByFolder.get(folder.id) ?? 0} lista(s)</span></span>
            </label>)}
          </div>
        </div>
