...
interface Props { mode: GlobalImportDestinationMode; destinationFolderName?: string }
...
export function AdvancedPromptBuilder({ mode, destinationFolderName }: Props) {
  ...
  const cards = useMemo(() => value.folders.reduce((total, folder) => total + folder.lists.reduce((sum, list) => sum + list.cardCount, 0), 0), [value.folders]);
  const setField = <K extends keyof PromptBuilderModel>(key: K, next: PromptBuilderModel[K]) => setValue((current) => ({ ...current, [key]: next }));

  const generate = () => setPreview(buildGlobalImportPrompt({
    mode,
