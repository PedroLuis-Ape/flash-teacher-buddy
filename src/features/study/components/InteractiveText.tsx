...
const GLOSSARY_OPEN_EVENT = "app-piteco:glossary-open";
      ...
      {segments.map((segment, index) => (
        segment.matches.length > 0 ? (
          <LayeredHintToken
            ...
            tokenId={`${segment.startIndex}-${segment.endIndex}-${index}`}
            value={segment.value}
...
function LayeredHintToken({
  ...
  useEffect(() => {
    const closeForAnotherToken = (event: Event) => {
      if ((event as CustomEvent<string>).detail !== tokenId) setOpen(false);
    };
    window.addEventListener(GLOSSARY_OPEN_EVENT, closeForAnotherToken);
    return () => window.removeEventListener(GLOSSARY_OPEN_EVENT, closeForAnotherToken);
  }, [tokenId]);
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    if (nextOpen) {
      window.dispatchEvent(new CustomEvent(GLOSSARY_OPEN_EVENT, { detail: tokenId }));
    }
    setOpen(nextOpen);
  };
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent
        ...
        sideOffset={8}
        collisionPadding={12}
        className="w-[min(18rem,calc(100vw-1.5rem))] max-h-[min(20rem,60vh)] overflow-y-auto p-0"
        onPointerDown={(event) => event.stopPropagation()}
        onClick={(event) => event.stopPropagation()}
      >
        <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b bg-popover px-3 py-2">
          <div className="min-w-0">
            <p className="truncate text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Significado</p>
          ...
          {layerCount > 1 && (
            <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-primary/10 px-2 py-1 text-[10px] font-medium text-primary">
              <Layers3 className="h-3 w-3" />{layerCount}
            </span>
          )}
        </div>
        ...
        <div className="divide-y">
          {matches.map((match) => (
            <div key={`${match.key}-${match.startIndex}-${match.endIndex}`} className="space-y-1.5 px-3 py-2.5">
              {uniqueTranslations(match).map((translation, index) => (
