...
function uniqueTranslations(match: LayeredHintMatch): DisplayTranslation[] {
  ...
  for (const translation of match.translations) {
    ...
    const item = map.get(key) ?? { text: cleanText, notes: [], sources: new Set<string>() };
    ...
    item.sources.add(translation.source);
    map.set(key, item);
  }
    ...
    <div className="max-h-[min(28rem,calc(100dvh-7rem))] overflow-y-auto bg-background text-foreground sm:max-h-[min(22rem,66vh)]">
      <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b bg-background px-3 py-2">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {onClose && (
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" onClick={onClose} aria-label="Fechar glossário">
              <X className="h-4 w-4" />
            </Button>
          )}
        </div>
      ...
      <div className="divide-y">
              ...
              {translations.map((translation, index) => (
                <div key={`${normalizeLayeredText(translation.text)}-${index}`}>
                  ...
                  {translation.notes.length > 0 && (
                    <p className="mt-1 text-xs italic leading-relaxed text-muted-foreground">{translation.notes.join(" · ")}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {hiddenCount > 0 && (
          <p className="px-3 py-2 text-xs text-muted-foreground">+{hiddenCount} camada(s) relacionada(s) ocultas para manter a leitura limpa.</p>
        )}
      </div>
    </div>
  );
    ...
    <>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          ...
          sideOffset={8}
          collisionPadding={12}
          className="hidden w-[min(20rem,calc(100vw-1.5rem))] overflow-hidden rounded-xl border bg-background p-0 shadow-xl sm:block"
          onPointerDown={(event) => event.stopPropagation()}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          hideClose
          className="fixed bottom-0 left-0 top-auto z-50 max-h-[calc(100dvh-2rem)] w-full max-w-none translate-x-0 translate-y-0 rounded-t-2xl border bg-background p-0 pb-[max(env(safe-area-inset-bottom),0.75rem)] shadow-2xl data-[state=open]:slide-in-from-bottom-full sm:hidden"
          onPointerDown={(event) => event.stopPropagation()}
          onClick={(event) => event.stopPropagation()}
        >
