...
const I18N_RESOURCE_DIR = resolve(root, "src", "i18n", "resources");
const catalogCopyCache = new Map();

/**
 * Copy do catálogo vem do i18n real (`src/i18n/resources/<locale>/home.json`).
 * Antes havia um dicionário paralelo aqui, e os 4 locales não-pt-BR serviam
 * texto em português no HTML pré-renderizado.
 */
export function catalogCopy(locale) {
  if (catalogCopyCache.has(locale)) return catalogCopyCache.get(locale);
  let publicCatalog = {};
  try {
    publicCatalog =
      JSON.parse(readFileSync(resolve(I18N_RESOURCE_DIR, locale, "home.json"), "utf8")).publicCatalog ?? {};
  } catch {
    publicCatalog = {};
  }
  const copy = {
    title: publicCatalog.title ?? "Materiais para praticar inglês",
    intro: publicCatalog.description ?? "",
    empty: [publicCatalog.emptyCurationTitle, publicCatalog.emptyCurationBody]
      .filter(Boolean)
      .join(" "),
  };
  catalogCopyCache.set(locale, copy);
  return copy;
}
...
function assertGeneratedCatalog(html, path, locale) {
  const canonical = absolute(path);
  const canonicals = html.match(/<link rel="canonical"[^>]*>/gi) ?? [];
  if (canonicals.length !== 1) throw new Error(`canonical duplicado ou ausente em ${path}`);
  if (!canonicals[0].includes(canonical)) throw new Error(`canonical incorreto em ${path}`);
  ...
  if (!/<h1[^>]*>[^<]+<\/h1>/i.test(html)) throw new Error(`H1 ausente em ${path}`);
  const copy = catalogCopy(locale);
  const hasList = html.includes("data-catalog-items");
  const hasEmpty = Boolean(copy.empty) && html.includes(copy.empty);
  if (!hasList && !hasEmpty) {
...
async function main() {
  ...
  for (const catalog of catalogs ?? []) {
    ...
    html = withContent;
    html = `<!-- Generated by scripts/prerender-public-materials.mjs for ${path} -->\n${html}`;
    assertGeneratedCatalog(html, path, catalog.locale);
    const destination = resolve(distDir, path.slice(1), "index.html");
    mkdirSync(dirname(destination), { recursive: true });
    writeFileSync(destination, html, "utf8");
    generatedPaths.push(path);
