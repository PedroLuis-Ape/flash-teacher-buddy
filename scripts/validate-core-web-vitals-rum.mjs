...
const statusPagePath = resolve(root, "src/pages/SystemStatus.tsx");
const statusCardPath = resolve(root, "src/components/system/CoreWebVitalsStatusCard.tsx");
...
assert.ok(migration.includes("'-Infinity'::double precision"));
assert.ok(migration.includes("'Infinity'::double precision"));
assert.ok(!migration.includes("isfinite("));
...
assert.ok(statusPage.includes('import { CoreWebVitalsStatusCard }'));
assert.ok(statusPage.includes("<CoreWebVitalsStatusCard />"));
